...
results['Fairness - Logistic Regression after only Reweighing'] = calculate_metrics(y_test, y_pred_reweighed, dataset_test, dataset_pred_reweighed,
                                                          privileged_groups, unprivileged_groups)

# --- Apply Adversarial Debiasing ---
# Set up AdversarialDebiasing model with both sensitive attributes
...
adversarial_model = AdversarialDebiasing(
    ...
    debias=True,
    num_epochs=50,  # You can adjust this based on your preference
    sess=sess
)
...
# Predict on the test set using the adversarial debiasing model
y_pred_adversarial = adversarial_model.predict(dataset_test).labels

# Create dataset for adversarial debiasing predictions
dataset_pred_adversarial = dataset_test.copy()
dataset_pred_adversarial.labels = np.array(y_pred_adversarial).reshape(-1, 1)

# Evaluate the model with adversarial debiasing
results['Fairness - After only Adversarial Debiasing'] = calculate_metrics(y_test, y_pred_adversarial, dataset_test, dataset_pred_adversarial,
                                                                      privileged_groups, unprivileged_groups)

# --- Apply Equalized Odds Post-Processing ---
eqodds_postprocessing = EqOddsPostprocessing(
    privileged_groups=privileged_groups,  # Using both sensitive attributes
    unprivileged_groups=unprivileged_groups
)

# Fit the EqOddsPostprocessing model to the test set (true labels) and the predictions made by the logistic model
eqodds_postprocessing = eqodds_postprocessing.fit(dataset_test, dataset_pred)

# Post-process predictions to ensure Equalized Odds fairness
...
# Evaluate the model with Equalized Odds Post-Processing for fairness
results['Fairness - Logistic Regression after only Equalized Odds Post-Processing'] = calculate_metrics(y_test, y_pred_eqodds, dataset_test, dataset_pred_eqodds,
                                                                            privileged_groups, unprivileged_groups)
