...
# privileged_groups = [{'AGE_GROUP': 0, 'GENDER': 0}, {'AGE_GROUP': 1, 'GENDER': 0},  {'AGE_GROUP': 0, 'GENDER': 1}]
# unprivileged_groups = [{'AGE_GROUP': 1, 'GENDER': 1}]
...
def calculate_metrics(y_true, y_pred, dataset_test, dataset_pred):
    ...
    roc = roc_auc_score(y_true, rf_model.predict_proba(X_test)[:, 1])
...
# Predicciones con el modelo reweighed
y_pred_reweighing = rf_model.predict(X_test)
...
# Convertir los datos en formato adecuado para AdversarialDebiasing
dataset_train = BinaryLabelDataset(favorable_label=0, unfavorable_label=1,
                                    df=pd.concat([X_train_resampled, y_train_resampled], axis=1),
                                    label_names=['DEFAULT'],
                                    protected_attribute_names=sensitive_features)

# Crear el modelo de AdversarialDebiasing
adv_debiasing = AdversarialDebiasing(privileged_groups=privileged_groups, 
                                     unprivileged_groups=unprivileged_groups, 
                                     scope_name='AdversarialDebiasing', 
                                     debias=True, 
                                     sess=sess)

# Entrenar el modelo con Adversarial Debiasing
adv_debiasing.fit(dataset_train)

# Hacer predicciones con el modelo de Adversarial Debiasing
y_pred_proba_adv = adv_debiasing.predict_proba(dataset_test.features)[:, 1]
y_pred_adv = (y_pred_proba_adv > A).astype(int)  # Aplicar el mismo umbral

# Actualizar etiquetas del dataset de predicción
dataset_pred.labels = y_pred_adv.reshape(-1, 1)

# Calcular métricas para Adversarial Debiasing
results['Adversarial Debiasing'] = calculate_metrics(y_test, y_pred_adv, dataset_test, dataset_pred)



print(results)

# print(np.any(np.isnan(dataset_pred.features)))
# print(np.any(np.isinf(dataset_pred.features)))


# # Post-procesamiento con Equalized Odds
# postprocessing = EqOddsPostprocessing(unprivileged_groups=[{'GENDER': 1}], privileged_groups=[{'GENDER': 0}])
# postprocessing.fit(dataset_test, dataset_pred)
# y_pred_post = postprocessing.predict(dataset_pred).labels
# results['Postprocessing (Equalized Odds)'] = calculate_metrics(y_test, y_pred_post, dataset_test, dataset_pred, sensitive_features)

# Combinar métodos
# Aplicar combinación aquí si es necesario
